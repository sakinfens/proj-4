...
if ($_REQUEST['action'] === 'index') {
  ...
  $new_meal = new Post(null, $body_object->title, $body_object->image, $body_object->instructions);
  $all_meals = Meals::create($new_meal);
  echo json_encode($all_meals);
} else if ($_REQUEST['action'] === 'update'){
  $request_body = file_get_contents('php://input');
  $body_object = json_decode($request_body);
  $updated_meal = new Meal($_REQUEST['id'], $body_object->title, $body_object->image, $body_object->instructions);
